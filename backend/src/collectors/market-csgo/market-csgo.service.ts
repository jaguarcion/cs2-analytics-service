  ...
  async fetchActiveTrades(): Promise<MarketTrade[]> {
    try {
      ...
      return items.map((item: any) => ({
        ...
        asset_id: item.asset_id || item.assetid ? String(item.asset_id || item.assetid) : undefined,
        class_id: item.classid || item.class_id ? String(item.classid || item.class_id) : undefined,
        instance_id: item.instanceid || item.instance_id ? String(item.instanceid || item.instance_id) : undefined,
  ...
  async fetchHistory(): Promise<MarketTrade[]> {
    try {
      ...
      return items.map((item: any) => ({
        ...
        asset_id: item.asset_id || item.assetid ? String(item.asset_id || item.assetid) : undefined,
        class_id: item.class || item.classid || item.class_id ? String(item.class || item.classid || item.class_id) : undefined,
        instance_id: item.instance || item.instanceid || item.instance_id ? String(item.instance || item.instanceid || item.instance_id) : undefined,
        settlement: item.settlement ? Number(item.settlement) : undefined,
        event: item.event,
        stage: item.stage,
